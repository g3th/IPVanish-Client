...
class TabsLists:
	...
	def buttonEvent(self):
		if not self.credentials:
			...
			try:
				...
				if selected == 0:
					selection_one = str(self.list_of_nations.curselection()).split("(")[1].strip(",)")
					nationCity = servers.nations()[int(selection_one)]
					selection = servers.nationsPaths()[int(selection_one)]

				if selected == 1:
					selectionTwo = str(self.list_of_cities.curselection()).split("(")[1].strip(",)")
	...
	def connect(self, info, city_selection):
		...
		nmcli_connection_user_name = ['nmcli', 'connection', 'modify', self.random_server, 'vpn.user-name',
									  str(data[0]).strip("\n")]
		nmcli_connection_password = ['nmcli', 'connection', 'modify', self.random_server, 'vpn.secrets',
									 'password=' + data[1]]

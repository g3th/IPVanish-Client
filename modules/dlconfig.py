		...
		while True:
			try:
				if len(os.listdir(self.directory)) > 1:
					...
					self.delete_and_download_again_button = Button(self.tab_four, text = 'Delete and Re-Download Configs', command = self.delete_and_download_again)
					self.delete_and_download_again_button.place(x=20,y=90)				
					self.progress_bar.destroy()
					break
				else:
					self.configs_download_button = Button(self.tab_four, text='Download Open-VPN Config Files',command=self.threaded_downloads)
